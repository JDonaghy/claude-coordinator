...
- **INVARIANT: every remote agent's `~/.coord-venv` MUST be a PyPI install (`pip install claude-coordinator`), never editable.** This is the single most-recurring fleet failure — it has bitten us repeatedly (precision again on 2026-06-07). When the install is PyPI, `coord agent update` runs `pip install --upgrade` and a released `vX.Y.Z` lands cleanly. When it's editable, update silently downgrades to **`editable (git pull)` mode** — it `git pull`s a local checkout instead of pulling the release, so version bumps never propagate and the agent often "did not come back."
  - **Root cause (what keeps undoing it):** a `~/src/claude-coordinator` git checkout exists on the agent machine **and** got `pip install -e .`'d into `~/.coord-venv`. The #402 PATH-strip only stops *workers* from poisoning the venv via bare `pip`; it does **nothing** about a pre-existing editable install or a human/setup step running `pip install -e .`. As long as that source checkout sits on an agent machine, editable drift recurs. **Remote agent machines should not have `~/src/claude-coordinator` at all** — only the coordinator-development machine does.
  - **Detect drift (do this proactively, e.g. at session start / before relying on a release):**
    ...
    ```
  - **Fix (convert to PyPI, then prevent recurrence):**
    ```bash
    ssh <host> '~/.coord-venv/bin/pip uninstall -y claude-coordinator \
      && ~/.coord-venv/bin/pip install --upgrade claude-coordinator'
    # then remove the source checkout so it can't be re-editable-installed:
    ssh <host> 'rm -rf ~/src/claude-coordinator'   # agent machines only — NOT the dev machine
    # then restart: coord agent update --machine <host>  (now runs pip --upgrade), or restart the service
    ```
  Full recovery + first-time-install steps in [`docs/AGENT_OPERATIONS.md`](docs/AGENT_OPERATIONS.md). When `coord agent update` reports `did not come back`, `editable (git pull)`, or the version won't advance, assume editable drift and run the detect/fix above.
