...
- **The review loop is ~29% of fleet spend — and the reviewer is not the lever (#2132, measured 2026-08-08 → 08-11, 393 legs / $907.04).** A 29% request-changes rate drove 35 fix legs and 54 re-review legs ($76.84), making the request-changes → fix → re-review loop several times larger than the sonnet-vs-opus gap everyone reaches for first ($7.30 vs $4.64 per work leg). **Do not act on that by cutting the review.** Classifying 27 of those blocking verdicts found **74% real behavioural defects, 18.5% missing-required-test, 0% style nits, 0% false positives** — "the reviewer is too strict" is measured *false*. `pipeline.max_review_iterations` is a **dead lever**: observed depth never exceeded 2 against a default of 5, so lowering it to 3 saves nothing and lowering it to 2 strands round-2 fixes on a human. Six of the 27 were the reviewer catching a **fresh regression introduced by a fix leg** (#2062 dropped a log fallback; #2100 turned "gate that cannot fail" into "gate that can never pass"; #2119 set an env var on the wrong unit) — fixes made under pressure introduce new defects, which is an argument for the gate, not against it. The one actionable lever is the mechanically-detectable "user-visible diff, zero test files" pattern (#2192). Re-measure with `coord usage --by-issue` or the DB directly — never the default view (#2128).
