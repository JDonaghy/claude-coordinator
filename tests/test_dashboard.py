...
class TestPipelineActionRecordReviewVerdict:
    ...
    def test_approve_verdict_persists(self) -> None:
        client = _client()
        with (
            patch("coord.dashboard.server.load_board", return_value=self._board_with_done()),
        ...
        data = r.json()
        assert data["ok"] is True
        mock_prf.assert_called_once_with("work001", "approve", "LGTM — code is clean.")

    def test_request_changes_verdict_persists(self) -> None:
        client = _client()
        with (
            patch("coord.dashboard.server.load_board", return_value=self._board_with_done()),
            patch("coord.notify._persist_review_findings") as mock_prf,
        ...
        mock_prf.assert_called_once_with(
            "work001", "request-changes", "Missing tests on the new endpoint.",
        )
