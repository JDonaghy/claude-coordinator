...
| `coord/reports.py`, `coord/commands/report.py` | Report engine (#1742): `REPORTS` registry, pure `fold_issue_activity()`, paginated audit fetch; `coord report` CLI + `GET /report`, `GET /report/{id}`. |
