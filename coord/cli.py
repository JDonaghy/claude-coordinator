    ...
    if interactive:
        ...
        if smoke_of is not None:
            ...
            INTERACTIVE_SMOKE_SYSTEM_PROMPT = (
                ...
                "- ALWAYS pull the prebuilt artifact first with "
                "`coord pull-artifact <work_aid>`. Do NOT build from source "
                "(`cargo build`, `make`, etc.) — the work-stage worker already "
                "built it. Only fall back to building if pull-artifact fails.\n"
                "- Do NOT run the project's full build/test suite (`cargo test`, "
                "`pytest`, `make test`, …). The work-stage worker already ran it "
                "before finishing, and it can pin the operator's CPU for no new "
                "signal. The Test stage is **black-box + user smoke**: validate "
                "the running app's behavior against the issue, NOT a re-run of "
                "unit/integration tests.\n"
                "- You MAY run git (read-only), RUN the pulled artifact, and the "
                ...
                "2. Pull the prebuilt artifact for this branch with "
                "`coord pull-artifact <work_aid>` (do this first — do NOT rebuild "
                "from source, and do NOT run the full test suite; both already "
                "happened at the work stage).\n"
