        ...
        ``enforces_deny_list=True``
            **SAFETY GATE — flipped in #1705, justified by real runs, not
            ...
            2. Enforcement is proven **end-to-end against the real
               opencode 1.18.11 binary**, not asserted from argv, by three
               named tests in ``tests/test_providers.py`` that each
               ``opencode run`` a real task and inspect the resulting
               NDJSON log for a genuine tool-call denial:
               ``test_opencode_work_agent_blocks_gh_end_to_end`` (a ``gh
               issue list`` bash call), ``test_opencode_work_agent_blocks_external_directory_edit_end_to_end``
               (an edit outside the worktree), and
               ...
               edit/git use.  All four are skipped (not failed) when the
